...
import {
  profileUnitTableHeaders,
  profileUnitRecordFields,
} from "../../utils/MostUsed";
...
function ProfilePreviousUnit({ id, UnitData, setProfileDate }) {
  ...
  const handleAddNewRow = async () => {
    try {
      ...
      setProfileDate((prev) => ({
        ...prev,
        previous_units: [...prev.previous_units, newRecord],
      }));
  ...
  const handleUpdateRow = async () => {
    try {
      const response = await fetch(
        `${ip}/userData/previous-units/${newRow.id}/`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setProfileDate((prev) => ({
        ...prev,
        previous_units: prev.previous_units.map((item) =>
  ...
  const handleDeleteRow = (recordId) => {
    showConfirmSnackbar(
      ...
      async () => {
        try {
          const response = await fetch(
            `${ip}/userData/previous-units/${recordId}/`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
          ...
          setProfileDate((prev) => ({
            ...prev,
            previous_units: prev.previous_units.filter(
  ...
  return (
    <>
      ...
      <AddModal
        ...
        title="اضافة وحدة"
        fields={profileUnitRecordFields}
        newRow={newRow}
...
export default ProfilePreviousUnit;
