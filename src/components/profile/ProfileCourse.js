...
import {
  profileCourseRecordFields,
  profileCourseTableHeaders,
} from "../../utils/MostUsed";
...
function ProfileCourse({ id, CourseData, setProfileDate }) {
  ...
  const handleAddNewRow = async () => {
    try {
      const response = await fetch(`${ip}/userData/courses/`, {
        method: "POST",
        headers: {
      ...
      setProfileDate((prev) => ({
        ...prev,
        courses: [...prev.courses, newRecord],
      }));
    ...
    showConfirmSnackbar(
      ...
      async () => {
        try {
          ...
          setProfileDate((prev) => ({
            ...prev,
            courses: prev.courses.filter((item) => item.id !== recordId),
          }));
  ...
  return (
    <>
      ...
      <AddModal
        ...
        title="اضافة فرقة"
        fields={profileCourseRecordFields}
        newRow={newRow}
